...
fdescribe('PerspectiveComponent', () => {
  ...
  it('should fill form and submit a valid form', fakeAsync(() => {
    ...
    spyOn(component, 'submit').and.callFake(formGroup => {
      ...
      expect(participantValue.participantAnswers).toEqual(mockParticipant.participantAnswers);
  ...
  it('submit() should send to API and navigate to result', fakeAsync(() => {
    ...
    expect(mockApiPerspectiveProvider.useValue.submitPerspective).toHaveBeenCalled();
    expect(component.participant$.value).toEqual(mockParticipantWithPerspective);
    expect(mockRouterProvider.useValue.navigate).toHaveBeenCalledWith([RouteList.result])
  }));
});

function fillForm(radioGroupDebugElements: DebugElement[], emailInputElement: HTMLInputElement) {
  radioGroupDebugElements.forEach((group, index) => {
    ...
    radioLabelElements[mockParticipant.participantAnswers[index].score - 1].click();
  });
